 ...
 * NexPractice IDE.
 *
 * @module     local_learnlogic/problem
 * @copyright  2026 Nex Academy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
define(['jquery', 'core/ajax', 'core/notification'], function($, Ajax, Notification) {

    const SPLIT_KEY = 'learnlogic.np.split.v2';
    const THEME_KEY = 'learnlogic.np.theme.v1';
    const SIDEBAR_KEY = 'learnlogic.np.sidebar.v1';
    ...
    const applyTheme = (root) => {
        ...
        root.toggleClass('ll-np--dark', dark);
        root.attr('data-theme', dark ? 'dark' : 'light');
        document.body.classList.toggle('ll-ide-dark', dark);
        document.documentElement.classList.toggle('ll-ide-dark', dark);
        document.body.setAttribute('data-ll-theme', dark ? 'dark' : 'light');
        syncRemuiDark(dark);

        const label = dark ? (state.strings.dark || 'Dark') : (state.strings.light || 'Light');
        root.find('[data-region="theme-label"]').text(label);
        root.find('.ll-np__theme-ico').html(dark ? THEME_MOON : THEME_SUN);
        ...
        if (state.ace) {
            try {
                ...
                const host = root.find('[data-region="ace"], .ll-np__ace-stage');
    ...
    const clearPanelFocus = (root) => {
        root.removeClass('ll-np--focus-desc ll-np--focus-right ll-np--focus-editor');
    ...
    const clearUiLocks = (root) => {
        ...
        document.body.classList.remove('ll-ide-resizing', 'll-ide-resizing-col', 'll-ide-resizing-row');
    };
    ...
    const initAce = (root, cfg) => {
        ...
        try {
            ...
            inner.className = 'll-np__ace-inner';
    ...
    const saveDraft = (root) => {
        if (!state.problem || state.cfg.battle) {
            return;
        }
        Ajax.call([{
            methodname: 'local_learnlogic_save_draft',
    ...
    const ioCard = (label, body) =>
        '<div class="ll-np__iocard">' +
        '<div class="ll-np__iocard-head">' +
        '<span>' + esc(label) + '</span>' +
        '<button type="button" class="ll-np__copy" data-copy="' + esc(body) + '" title="Copy">⧉</button>' +
        '</div>' +
        '<pre class="ll-np__iocard-pre">' + esc(body) + '</pre></div>';
    ...
    const renderSampleView = (root) => {
        ...
        root.find('[data-region="sample-chips"]').html(
            samples.map((sample, idx) =>
                '<button type="button" class="ll-np__chip' + (idx === state.sampleIndex ? ' is-active' : '') +
                '" data-sample-idx="' + idx + '">' +
                (idx === state.sampleIndex ? '<span class="ll-np__chip-check">✓</span> ' : '') +
                'Test ' + (idx + 1) + '</button>'
            ).join('')
        );
        ...
        if (!samples.length) {
            root.find('[data-region="sample-view"]').html(
                '<p class="ll-np__empty">' + esc(s.nosamples || 'No sample tests.') + '</p>'
            );
            return;
        }
        const sample = samples[state.sampleIndex];
        root.find('[data-region="sample-view"]').html(
            '<div class="ll-np__iocards">' +
            ioCard(s.input || 'Input', sample.stdin) +
            ioCard(s.output || 'Output', sample.expected) +
            '</div>' +
            (sample.explanation
                ? '<div class="ll-np__explain"><strong>' + esc(s.explanation || 'Explanation') +
                    ':</strong> <div class="ll-np__explain-body">' + sample.explanation + '</div></div>'
                : '')
        );
    };
    ...
    const renderExamplesInDesc = (root) => {
        ...
        const html = samples.map((sample, idx) =>
            '<article class="ll-np__example">' +
            '<h3 class="ll-np__example-title">Example ' + (idx + 1) + ':</h3>' +
            '<div class="ll-np__iocards">' +
            ioCard(s.input || 'Input', sample.stdin) +
            ioCard(s.output || 'Output', sample.expected) +
            '</div>' +
    ...
    const renderResults = (root, payload) => {
        ...
        if (payload.statusLabel || payload.allPassed || payload.message) {
            ...
            html += '<div class="ll-np__outcome ' + (ok ? 'is-ok' : 'is-fail') + '">' +
                esc(ok ? (s.accepted || 'Accepted') : (payload.statusLabel || s.wronganswer || 'Wrong Answer'));
            if (payload.passed != null) {
                html += ' <span class="ll-np__pass">' + esc(payload.passed) + ' / ' + esc(payload.total) + '</span>';
            }
            html += '</div>';
        }
        if (payload.message) {
            html += '<div class="ll-np__err">' + esc(payload.message) + '</div>';
        }
        results.forEach((r, idx) => {
            const hidden = r.display === 'hidden';
            if (hidden) {
                // Pass/fail only — never reveal stdin/expected/output for hidden cases.
                html += '<article class="ll-np__result ll-np__result--hidden ' +
                    (r.isCorrect ? 'is-ok' : 'is-fail') + '">' +
                    '<header>Hidden case ' + (idx + 1) +
                    ' — ' + esc(r.isCorrect ? (s.accepted || 'Passed') : (s.wronganswer || 'Failed')) +
                    '</header></article>';
                return;
            }
            const empty = !(r.actual && String(r.actual).length);
            html += '<article class="ll-np__result ' + (r.isCorrect ? 'is-ok' : 'is-fail') + '">' +
                '<header>Case ' + (idx + 1) + '</header>' +
                '<div class="ll-np__iocards">' +
                    ioCard(s.input || 'Input', r.input) +
                    ioCard(s.expected || 'Expected', r.expected) +
                '</div>' +
                '<div class="ll-np__iocard"><div class="ll-np__iocard-head">' +
                esc(empty ? (s.nooutput || 'No Output') : (s.youroutput || 'Your Output')) +
                '</div><pre class="ll-np__iocard-pre' + (empty ? ' is-empty' : '') + '">' +
                (empty ? esc(s.nooutput || 'No Output') : esc(r.actual)) + '</pre></div>' +
                (r.stderr ? '<pre class="ll-np__iocard-pre">' + esc(r.stderr) + '</pre>' : '') +
                '</article>';
        });
        const panel = root.find('[data-region="results"]');
        panel.html(html || '<p class="ll-np__empty">No results.</p>');
    ...
    const loadSubs = (root) => {
        Ajax.call([{
            ...
            root.find('[data-region="subs"]').html(
                rows
                    ? '<table class="ll-np__table"><thead><tr><th>When</th><th>Status</th><th>Tests</th><th>Lang</th></tr></thead><tbody>' +
                        rows + '</tbody></table>'
                    : '<p class="ll-np__empty">No submissions yet.</p>'
            );
            return null;
    ...
    const renderSidebar = (root) => {
        ...
        root.find('[data-region="sidebar-list"]').html(
            items.map((p, idx) =>
                '<a class="ll-np__plist-item' + (Number(p.id) === Number(currentId) ? ' is-active' : '') +
                '" href="' + esc(p.url || (state.cfg.listUrl.replace(/index\.php.*/, 'problem.php?id=') + p.id)) +
                '" data-problemid="' + esc(p.id) + '">' +
                '<span class="ll-np__plist-num">' + esc(p.number || (idx + 1)) + '</span>' +
                '<span class="ll-np__plist-name">' + esc(p.name) + '</span>' +
                '<span class="ll-np__plist-diff ll-np__plist-diff--' + esc(p.difficulty) + '">' +
                esc(diffLabel(p.difficulty)) + '</span></a>'
            ).join('') || '<p class="ll-np__empty">No problems.</p>'
        );
    };
    ...
    const toastXp = (root, xp) => {
        ...
        const id = 'll-toast-' + Date.now();
        const el = $(
            '<div class="ll-np__toast ll-np__toast--xp" id="' + id + '" role="status">' +
            '<div class="ll-np__toast-ico" aria-hidden="true">✦</div>' +
            '<div class="ll-np__toast-body">' +
            '<div class="ll-np__toast-title">' + esc(state.strings.accepted || 'Accepted') + '</div>' +
            '<div class="ll-np__toast-msg">' + esc(msg) + '</div>' +
            '</div>' +
            '<button type="button" class="ll-np__toast-close" aria-label="Close">×</button>' +
            '</div>'
        );
        host.append(el);
        ...
        el.find('.ll-np__toast-close').on('click', remove);
        window.setTimeout(remove, 4200);
    };

    const icoThumb = () =>
        '<svg class="ll-np__ico" viewBox="0 0 16 16" width="14" height="14" aria-hidden="true" focusable="false">' +
    ...
    const fillProblem = (root, p) => {
        ...
        const tags = (p.tags || []).map((t) =>
            '<span class="ll-np__tag">' + esc(t.name) + '</span>'
        ).join('');
        const companies = (p.companies || []).map((t) =>
            '<span class="ll-np__tag ll-np__tag--company">' + esc(t.name) + '</span>'
        ).join('');
        root.find('[data-region="meta"]').html(
            '<span class="ll-np__diff ll-np__diff--' + esc(p.difficulty) + '">' + esc(p.difficulty) + '</span>' +
            '<span class="ll-np__stat" title="Solvers">' + icoThumb() +
                '<strong>' + esc(p.solvers || 0) + '</strong></span>' +
            '<span class="ll-np__stat" title="Success rate">' + icoRate() +
                '<strong>' + esc(acceptance) + '</strong></span>' +
            (tags || companies ? '<span class="ll-np__tags">' + tags + companies + '</span>' : '')
        );
        root.find('[data-region="statement"]').html(
            '<div class="ll-np__qtext">' + (p.statement || '') + '</div>'
        );
        ...
        renderSidebar(root);
        document.title = (p.name || 'Problem') + ' · NexPractice';
    };

    const renderSolution = (root) => {
        const p = state.problem;
        const host = root.find('[data-region="solution"]');
        if (!host.length) {
            return;
        }
        const solutions = (p && p.solutions) || [];
        const lang = state.language || (p && p.defaultlanguage) || '';
        let sol = solutions.find((row) => row.language === lang);
        if (!sol && solutions.length) {
            sol = solutions[0];
        }
        if (!sol || (!sol.code && !sol.explanation)) {
            host.html('<p class="ll-np__empty">' + esc(state.strings.nosolution || 'No editorial solution yet.') + '</p>');
            return;
        }
        let html = '';
        if (sol.code) {
            html += '<div class="ll-np__solution-lang">' + esc(sol.language || lang) + '</div>';
            html += '<pre class="ll-np__solution-code"><code>' + esc(sol.code) + '</code></pre>';
        }
        if (sol.explanation) {
            html += '<div class="ll-np__solution-explain"><h3 class="ll-np__solution-heading">' +
                esc(state.strings.explanation || 'Explanation') + '</h3>' +
                '<div class="ll-np__solution-body">' + sol.explanation + '</div></div>';
        }
        host.html(html);
    };
    ...
    const bindResizers = (root) => {
        ...
        const startDrag = (kind, ev) => {
            ...
            document.body.classList.add('ll-ide-resizing', kind === 'col' ? 'll-ide-resizing-col' : 'll-ide-resizing-row');
    ...
    const setSidebarOpen = (root, open) => {
        root.toggleClass('ll-np--sidebar-open', open);
        root.toggleClass('ll-np--sidebar-closed', !open);
        try {
    ...
    const bind = (root, cfg) => {
        root.on('click', '[data-action="toggle-sidebar"]', () => {
            setSidebarOpen(root, !root.hasClass('ll-np--sidebar-open'));
        });
        ...
        root.on('click', 'a.ll-np__plist-item', function(e) {
        ...
        $(document).on('keydown.llnpfs', (e) => {
            if (e.key === 'Escape' && root.is('.ll-np--focus-desc, .ll-np--focus-right, .ll-np--focus-editor')) {
                clearPanelFocus(root);
        ...
        const run = (mode) => {
            ...
            let method;
            let callArgs;
            // Interview embeds reuse battle shell CSS but must call NexPractice judge APIs.
            if (state.cfg.battle && state.cfg.battle.battleId && !state.cfg.interview) {
                // Battle: submit decides the duel; sample/custom run stays non-deciding.
                if (mode === 'submit') {
                    method = 'local_nexbattleground_submit_code';
                    callArgs = {
                        battleid: state.cfg.battle.battleId,
                        language: args.language,
                        code: args.code
                    };
                } else if (mode === 'custom') {
                    method = 'local_learnlogic_run_code';
                    callArgs = args;
                } else {
                    method = 'local_nexbattleground_run_code';
                    callArgs = {
                        battleid: state.cfg.battle.battleId,
                        language: args.language,
                        code: args.code
                ...
                method = mode === 'submit' ? 'local_learnlogic_submit_code' : 'local_learnlogic_run_code';
                callArgs = mode === 'submit'
            ...
            Promise.resolve(Ajax.call([{methodname: method, args: callArgs}])[0])
                .then((payload) => {
                    // Battle submit returns flattened battle + judgeJson.
                    let judge = payload;
                    if (state.cfg.battle && mode === 'submit' && payload && payload.judgeJson) {
                        try {
                            judge = Object.assign({}, JSON.parse(payload.judgeJson), {
                    ...
                    } else if (state.cfg.battle && mode !== 'submit' && payload && payload.resultJson) {
                        try {
                            judge = JSON.parse(payload.resultJson);
                        } catch (e) {
                            judge = payload;
                        }
                    }
                    renderResults(root, judge);
                    if (mode === 'submit' && !state.cfg.battle) {
                        loadSubs(root);
                        toastXp(root, payload && payload.xpAwarded);
                    }
                    if (mode === 'submit' && state.cfg.battle && typeof state.cfg.battle.onResult === 'function') {
                        state.cfg.battle.onResult(payload);
                    }
                    if (mode === 'submit' && state.cfg.interview && typeof state.cfg.interview.onResult === 'function') {
                        state.cfg.interview.onResult(judge || payload);
                    }
                    return null;
                })
                .catch((err) => {
        ...
        root.off('click.llnp-run')
            .on('click.llnp-run', '[data-action="run"]', () => run('sample'));
        root.off('click.llnp-custom')
            .on('click.llnp-custom', '[data-action="run-custom"]', () => run('custom'));
        root.off('click.llnp-submit')
            .on('click.llnp-submit', '[data-action="submit"]', () => run('submit'));

        bindResizers(root);
    };

    const reloadProblem = function(problemId, cfgPatch) {
        const root = $('[data-region="ll-ide"]');
        if (!root.length || !problemId) {
            return;
        }
        if (cfgPatch) {
            state.cfg = Object.assign({}, state.cfg || {}, cfgPatch);
        }
        root.attr('data-problemid', String(problemId));
        Ajax.call([{
            methodname: 'local_learnlogic_get_problem',
            args: {problemid: problemId}
        }])[0].then((p) => {
            state.problem = p;
            state.language = p.defaultlanguage || ((p.languages[0] || {}).language) || 'python3';
            fillProblem(root, p);
            initAce(root, state.cfg || {});
            applyTheme(root);
    ...
    const init = function(cfg) {
        ...
        document.body.classList.add('has-ll-ide');
        if (cfg.battle || cfg.interview) {
            root.addClass('ll-np--battle');
            document.body.classList.add('ll-np-battle');
        }
        if (cfg.interview) {
            root.addClass('ll-np--interview');
            document.body.classList.add('ll-np-interview');
        }
        ...
        // Battle / interview mode: never show the problem list sidebar.
