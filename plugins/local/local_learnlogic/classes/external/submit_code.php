...
class submit_code extends external_api {
    ...
    public static function execute(int $problemid, string $language, string $code): array {
        ...
        $problem = $DB->get_record('local_learnlogic_problem', [
            'id' => $params['problemid'],
            'status' => 'ready',
        ], '*', MUST_EXIST);
        ...
        $sid = $DB->insert_record('local_learnlogic_submission', (object) [
            'userid' => (int) $USER->id,
        ...
        // Never send hidden case I/O (or stderr) to the browser.
