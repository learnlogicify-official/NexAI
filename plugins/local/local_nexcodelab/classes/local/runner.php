...
/**
 * CodeRunner-backed execution for NexCodeLab problems.
 *
...
class runner {
    ...
    public static function coderunner_available(): bool {
        ...
        try {
            ...
            debugging('NexCodeLab CodeRunner bootstrap: ' . $e->getMessage(), DEBUG_DEVELOPER);
            return false;
        }
    ...
    /**
     * Load the live CodeRunner question for a NexCodeLab problem.
     *
     * Always follows the question-bank entry to the **latest version**, so edits
     * in CodeRunner (new Moodle question id) show up in NexCodeLab without re-import.
     *
    ...
    public static function load_problem_question($problem): \qtype_coderunner_question {
        ...
        $linked = (int) ($problem->sourcequestionid ?? 0) > 0;
        $qid = $linked ? (int) $problem->sourcequestionid : self::prototype_id($lang, 0);
        if ($qid < 1) {
            // Site prototype required for seeded challenges (sourcequestionid = 0).
            throw new \moodle_exception('noprototype', 'local_nexcodelab', '', $lang);
        }
        ...
        if ($latest > 0 && $latest !== $qid) {
            $qid = $latest;
            // Only persist for *linked* challenges — never bind the site prototype onto
            // seeded rows (that would wipe local fixture tests).
            if ($linked && !empty($problem->id)) {
        ...
        // NexCodeLab runs straight into jobrunner, which requires $question->prototype.
    ...
    private static function touch_source_question_id(int $problemid, int $questionid): void {
        ...
        try {
            $DB->set_field('local_nexcodelab_problem', 'sourcequestionid', $questionid, ['id' => $problemid]);
            $DB->set_field('local_nexcodelab_problem', 'timemodified', time(), ['id' => $problemid]);
            // Linked problems must not keep stale copied local tests.
            if ($DB->get_manager()->table_exists('local_nexcodelab_testcase')) {
                $DB->delete_records('local_nexcodelab_testcase', ['problemid' => $problemid]);
            }
        } catch (\Throwable $e) {
     ...
     * attempts call get_prototype() during display; NexCodeLab must do the same
    ...
    public static function prototype_id(string $language, int $override = 0): int {
        ...
        return (int) (get_config('local_nexcodelab', 'prototype_' . $language) ?: 0);
    }
    ...
    public static function execute(
        ...
        $problem = $DB->get_record('local_nexcodelab_problem', ['id' => $problemid], '*', MUST_EXIST);
        $linked = (int) ($problem->sourcequestionid ?? 0) > 0;
        ...
        if ($mode === 'custom') {
            ...
            // Option A: site prototype + per-challenge local tests / fixtures.
            $testcases = self::local_testcases_for_mode((int) $problem->id, $problem, $mode === 'sample' ? 'sample' : 'all');
            if (empty($testcases)) {
        ...
        // Seeded DS stubs need a small driver so stdin CSV → clean_df/solve → stdout.
        if (!$linked && $mode !== 'custom') {
            $code = self::wrap_ds_driver($code);
        }

        if (empty($testcases)) {
            return [
                'success' => false,
                'message' => get_string('importnoneinbank', 'local_nexcodelab'),
                'results' => [],
                'allPassed' => false,
                'passed' => 0,
                'total' => 0,
    ...
    public static function prepare_question_for_run($question): void {
        ...
        if (method_exists($question, 'evaluate_question_for_display')) {
            try {
                ...
                debugging('NexCodeLab evaluate_question_for_display: ' . $e->getMessage(), DEBUG_DEVELOPER);
            }
        }
    ...
    public static function testcases_for_mode($question, string $mode): array {
        $precheckonly = 1; // qtype_coderunner\constants::PRECHECK
    ...
    /**
     * Build jobrunner testcases from local_nexcodelab_testcase (+ fixture CSV as stdin).
     *
     * Used when the challenge is not linked to a CodeRunner question and the site
     * python3 prototype supplies the sandbox/template only.
     *
     * @param int $problemid
     * @param \stdClass $problem
     * @param string $mode sample|all
     * @return \stdClass[]
     */
    public static function local_testcases_for_mode(int $problemid, $problem, string $mode): array {
        global $DB, $CFG;

        $rows = $DB->get_records('local_nexcodelab_testcase', ['problemid' => $problemid], 'sortorder ASC, id ASC');
        if (!$rows) {
            return [];
        }

        $fixturecsv = '';
        $rel = trim((string) ($problem->fixturepath ?? ''));
        if ($rel !== '') {
            $path = $CFG->dirroot . '/local/nexcodelab/' . ltrim(str_replace('\\', '/', $rel), '/');
            // Stay inside the plugin tree.
            $base = realpath($CFG->dirroot . '/local/nexcodelab');
            $real = realpath($path);
            if ($base && $real && strpos($real, $base) === 0 && is_readable($real)) {
                $fixturecsv = (string) file_get_contents($real);
            }
        }
        ...
        $order = 0;
        foreach ($rows as $row) {
            $display = strtolower((string) ($row->display ?? 'sample'));
            if ($mode === 'sample' && $display !== 'sample') {
                continue;
            }
            $stdin = (string) ($row->stdin ?? '');
            if (trim($stdin) === '' && $fixturecsv !== '') {
                $stdin = $fixturecsv;
            }
            $tc = self::make_testcase(
                $stdin,
                (string) ($row->expected ?? ''),
                ($display === 'sample') ? 'SHOW' : 'HIDE',
                '',
    ...
    /**
     * Append a stdin→DataFrame driver so function stubs grade via stdout exact match.
     *
     * @param string $code
     * @return string
     */
    public static function wrap_ds_driver(string $code): string {
        // Avoid double-wrapping if the learner already pasted the driver.
        if (strpos($code, '__ncl_main') !== false) {
            return $code;
        }
        $driver = <<<'PY'

# --- NexCodeLab DS driver (site prototype path) ---
import sys
import io
try:
    import pandas as pd
...
def __ncl_main():
    raw = sys.stdin.read()
    if "clean_df" in globals() and callable(clean_df) and pd is not None:
    ...
    if "solve" in globals() and callable(solve):
        if pd is not None and str(raw).strip():
...
PY;
    ...
    private static function run_tests_isolated($question, string $code, array $testcases, string $runlang) {
        ...
        foreach ($testcases as $tc) {
            $runner = new \qtype_coderunner_jobrunner();
            $one = $runner->run_tests($question, $code, [], [$tc], false, $runlang, false);
            if (!is_object($one)) {
                continue;
            }
    ...
    public static function partition_testcases($question): array {
        ...
        return [
            'samples' => self::clone_testcases($samples),
            'hidden' => self::clone_testcases($hidden),
        ];
    }
    ...
    public static function is_example_case($t): bool {
        ...
        // Explicit NexCodeLab / legacy marker only — not plain SHOW.
    ...
    private static function normalize_outcome($outcome, array $testcases): array {
        ...
        foreach ($testcases as $i => $tc) {
            ...
            $results[] = [
                'input' => (string) ($tc->stdin ?? ''),
                'expected' => (string) ($tc->expected ?? ''),
                'actual' => $actual,
                'isCorrect' => $iscorrect,
                'stderr' => $stderr,
                'status' => $status,
                'display' => ($tc->display ?? 'SHOW') === 'SHOW' ? 'sample' : 'hidden',
            ];
        }
