...
$PAGE->set_url(new moodle_url('/local/nexcodelab/submissions.php'));
$PAGE->set_context($context);
$PAGE->set_title(get_string('submissions', 'local_nexcodelab'));
local_nexcodelab_setup_page($PAGE);
$PAGE->requires->css('/local/nexcodelab/styles.css');
$PAGE->requires->js_call_amd('local_nexcodelab/submissions', 'init', [[]]);

$canmanage = has_capability('local/nexcodelab:manageproblems', $context);
$header = local_nexcodelab_header_context((int) $USER->id);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexcodelab/submissions', array_merge($header, [
    'canmanage' => $canmanage,
