...
$PAGE->set_url(new moodle_url('/local/nexcomm/leaderboard.php'));
$PAGE->set_context($context);
$PAGE->set_title(get_string('leaderboard', 'local_nexcomm'));
local_nexcomm_setup_page($PAGE);
$PAGE->requires->css('/local/nexcomm/styles.css');
$PAGE->requires->js_call_amd('local_nexcomm/leaderboard', 'init', [[]]);

$canmanage = has_capability('local/nexcomm:manage', $context);
$header = local_nexcomm_header_context((int) $USER->id);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexcomm/leaderboard', array_merge($header, [
