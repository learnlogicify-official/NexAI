/**
 * NexReports NexInterview — attempt ledger with scores and feedback links.
 *
 * @module     local_nexreports/nexinterview
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const syncStatusSelect = function(select, selected) {
        if (!select) {
            return;
        }
        const want = String(selected == null || selected === '' ? 'all' : selected);
        select.value = want;
        if (select.value !== want) {
            select.value = 'all';
        }
    ...
    const render = function(root, data) {
        ...
        fillNamedSelect(
            root.querySelector('[data-filter="track"]'),
            data.tracks || [],
            data.selectedtrack,
            root.getAttribute('data-label-all-tracks') || 'All tracks'
        );
        syncStatusSelect(root.querySelector('[data-filter="status"]'), data.selectedstatus);
        ...
        const host = root.querySelector('[data-region="interview-table"]');
        if (!host) {
            return;
        }
        ...
        const head = '<tr>' +
            ...
            '<th>' + esc(label(root, 'track', 'Track')) + '</th>' +
            '<th>' + esc(label(root, 'status', 'Status')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'score', 'Overall')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'conceptual', 'Conceptual')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'problemsolving', 'Problem-solving')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'coding', 'Coding')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'explanation', 'Explanation')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'communication', 'Communication')) + '</th>' +
            '<th class="nxr-iv-num">' + esc(label(root, 'independence', 'Independence')) + '</th>' +
            '<th>' + esc(label(root, 'started', 'Started')) + '</th>' +
            '<th>' + esc(label(root, 'completed', 'Completed')) + '</th>' +
            '<th>' + esc(label(root, 'feedback', 'Feedback')) + '</th>' +
            '</tr>';

        const dimCell = function(r, key) {
            const display = r[key + 'display'];
        ...
        const body = rows.map(function(r) {
            const feedback = r.feedbackurl
                ? ('<a href="' + esc(r.feedbackurl) + '">' + esc(r.feedbacklabel || label(root, 'feedback', 'View')) + '</a>')
                : dash;
            return '<tr>' +
                '<td>' + esc(r.rank) + '</td>' +
                '<td class="nxr-iv-learner"><a href="' + esc(r.url) + '">' + esc(r.firstname || '') + '</a></td>' +
                '<td>' + esc(r.lastname || '') + '</td>' +
                '<td>' + esc(r.username || '') + '</td>' +
                '<td>' + esc(r.institution || dash) + '</td>' +
                '<td>' + esc(r.yearofpassing || dash) + '</td>' +
                '<td>' + esc(r.department || dash) + '</td>' +
                '<td>' + esc(r.track || dash) + '</td>' +
                '<td>' + esc(r.status || dash) + '</td>' +
                '<td class="nxr-iv-num">' + esc(r.scoredisplay != null ? r.scoredisplay : dash) + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'conceptual') + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'problemsolving') + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'coding') + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'explanation') + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'communication') + '</td>' +
                '<td class="nxr-iv-num">' + dimCell(r, 'independence') + '</td>' +
                '<td>' + esc(r.started || dash) + '</td>' +
                '<td>' + esc(r.completed || dash) + '</td>' +
                '<td class="nxr-iv-feedback">' + feedback + '</td>' +
                '</tr>';
        }).join('');

        host.innerHTML =
            '<div class="nxr-table-wrap nxr-table-wrap--interview">' +
            '<table class="nxr-table nxr-table--interview">' +
            '<thead>' + head + '</thead><tbody>' + body + '</tbody></table></div>';
    };

    const init = function() {
        const root = document.querySelector('[data-region="nxr-interview"]');
        if (!root) {
            return;
        }
        ...
        const load = function() {
            ...
            const track = root.querySelector('[data-filter="track"]');
            const status = root.querySelector('[data-filter="status"]');
            const search = root.querySelector('[data-filter="search"]');
            Ajax.call([{
                methodname: 'local_nexreports_get_interview_attempts',
                args: {
                    ...
                    status: status ? (status.value || 'all') : 'all',
                ...
                const intendedTrack = data.selectedtrack || '';
                const intendedStatus = data.selectedstatus || 'all';
                window.setTimeout(function() {
                    ...
                    syncNamedSelect(root.querySelector('[data-filter="track"]'), intendedTrack);
                    syncStatusSelect(root.querySelector('[data-filter="status"]'), intendedStatus);
                ...
                url.searchParams.set('status', intendedStatus);
                url.searchParams.set('track', intendedTrack);
        ...
        const trackFilter = root.querySelector('[data-filter="track"]');
        if (trackFilter) {
            trackFilter.addEventListener('change', onFilterChange(false, false));
        }
        const statusFilter = root.querySelector('[data-filter="status"]');
        if (statusFilter) {
            statusFilter.addEventListener('change', onFilterChange(false, false));
        }
