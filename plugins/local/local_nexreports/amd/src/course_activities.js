/**
 * NexReports Course Activities Summary.
 *
 * @module     local_nexreports/course_activities
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const columns = function(root) {
        return [
            ...
            {key: 'name', label: label(root, 'activity', 'Activity'), type: 'str', sortKey: 'name', link: true},
            {key: 'type', label: label(root, 'type', 'Type'), type: 'str', sortKey: 'type'},
            {key: 'status', label: label(root, 'status', 'Status'), type: 'str', sortKey: 'status'},
            {key: 'learnerscompleted', label: label(root, 'completed', 'Completed'), type: 'num', sortKey: 'learnerscompleted'},
            {key: 'completionrate', label: label(root, 'completionrate', 'Completion %'), type: 'num', sortKey: 'completionrate', suffix: '%'},
            {key: 'totalgrade', label: label(root, 'totalgrade', 'Total grade'), type: 'num', sortKey: 'totalgrade'},
            {key: 'averagegrade', label: label(root, 'avggrade', 'Avg grade'), type: 'num', sortKey: 'averagegrade'},
            {key: 'totalvisits', label: label(root, 'visits', 'Visits'), type: 'num', sortKey: 'totalvisits'},
            {key: 'totaltimespent', label: label(root, 'timespent', 'Time spent'), type: 'num', sortKey: 'totaltimespent', duration: true},
        ];
    };
    ...
    const sortValue = function(row, col) {
        if (col.type === 'num') {
            ...
            if (col.duration && (raw == null || raw === '')) {
                raw = (Number(row.totaltimespentminutes) || 0) * 60;
            }
            const n = Number(raw);
    ...
    const cellValue = function(row, col, displayRank) {
        ...
        if (col.duration) {
            const secs = row.totaltimespent != null
                ? row.totaltimespent
                : ((Number(row.totaltimespentminutes) || 0) * 60);
    ...
    const init = function(cfg) {
        cfg = cfg || {};
        const root = document.querySelector('[data-region="nxr-course-activities"]');
        if (!root) {
            return;
        }
        ...
        const load = function(opts) {
            ...
            Ajax.call([{
                methodname: 'local_nexreports_get_course_activities_summary',
                args: {
                    courseid: courseid,
                    groupid: 0,
        ...
        root.addEventListener('click', function(e) {
            ...
            if (th && root.contains(th)) {
                ...
                if (key === sortKey) {
                    ...
                    sortKey = key;
                    sortDir = key === 'learnerscompleted' || key === 'completionrate' ||
                        key === 'totalgrade' || key === 'averagegrade' ||
                        key === 'totalvisits' || key === 'totaltimespent'
                        ? 'desc' : 'asc';
                }
                page = 1;
                paint();
