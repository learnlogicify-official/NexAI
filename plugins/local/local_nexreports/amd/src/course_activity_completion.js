/**
 * NexReports Course Activity Completion.
 *
 * @module     local_nexreports/course_activity_completion
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const columns = function(root) {
        return [
            ...
            {key: 'totalmark', label: label(root, 'totalmark', 'Total mark'), type: 'num', sortKey: 'totalmarkvalue'},
            {key: 'gradepercent', label: label(root, 'gradepercent', 'Grade %'), type: 'num', sortKey: 'gradepercentvalue', suffix: '%'},
            {key: 'passgrade', label: label(root, 'passmark', 'Pass mark'), type: 'num', sortKey: 'passgradevalue'},
    ...
    const fillActivitiesForCourse = function(select, courseid, options, selected) {
        if (!select) {
            return;
        }
        const key = String(courseid || 0);
        if (select.getAttribute('data-course') === key && select.options.length) {
            if (selected != null && select.querySelector('option[value="' + selected + '"]')) {
                select.value = String(selected);
            }
            return;
        }
    ...
    const init = function(cfg) {
        cfg = cfg || {};
        const root = document.querySelector('[data-region="nxr-course-activity-completion"]');
        if (!root) {
            return;
        }
        ...
        const load = function(opts) {
            ...
            const activity = root.querySelector('[data-filter="activity"]');
            ...
            const courseid = course && course.options.length
                ...
                : (coursePrimed || 0);
            let cmid = activity && activity.options.length
                ? (parseInt(activity.value, 10) || 0)
                : (cmPrimed || 0);
            ...
            Ajax.call([{
                methodname: 'local_nexreports_get_course_activity_completion',
                args: {
                    courseid: courseid,
                    cmid: cmid,
                    groupid: 0,
                ...
                fillActivitiesForCourse(
                    root.querySelector('[data-filter="activity"]'),
                ...
                const url = new URL(root.getAttribute('data-export-url'), window.location.origin);
                    ...
                    url.searchParams.set('cmid', String(data.selectedcmid || 0));
        ...
        if (course) {
            course.addEventListener('change', function() {
                ...
                const a = root.querySelector('[data-filter="activity"]');
        ...
        root.addEventListener('click', function(e) {
            ...
            if (th && root.contains(th)) {
                ...
                if (key === sortKey) {
                    ...
                    sortKey = key;
                    sortDir = key === 'grade' || key === 'gradepercent' || key === 'visits' ||
                        key === 'timespent' || key === 'completedon' || key === 'gradedon' ||
                        key === 'firstaccess' || key === 'lastaccess' || key === 'totalmark'
                        ? 'desc' : 'asc';
                }
                page = 1;
                paint();
