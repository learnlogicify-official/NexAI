/**
 * NexReports NexBattleGround — wins, win rate, and battle XP.
 *
 * @module     local_nexreports/nexbattleground
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const render = function(root, data) {
        ...
        const host = root.querySelector('[data-region="battle-table"]');
        if (!host) {
            return;
        }
        ...
        const head = '<tr>' +
            ...
            '<th class="nxr-pr-num">' + esc(label(root, 'battlexp', 'Battle XP')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'wins', 'Wins')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'losses', 'Losses')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'ties', 'Ties')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'battles', 'Battles')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'winrate', 'Win rate')) + '</th>' +
        ...
        const body = rows.map(function(r) {
            return '<tr>' +
                ...
                '<td class="nxr-pr-num">' + esc(r.battlexp) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.wins) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.losses) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.ties) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.battles) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.winrate) + '%</td>' +
    ...
    const init = function() {
        const root = document.querySelector('[data-region="nxr-battle"]');
        if (!root) {
            return;
        }
        ...
        const load = function() {
            ...
            Ajax.call([{
                methodname: 'local_nexreports_get_battle_leaderboard',
                args: {
