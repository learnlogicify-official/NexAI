/**
 * NexReports NexPractice — XP leaderboard with solve counts and streaks.
 *
 * @module     local_nexreports/nexpractice
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const render = function(root, data) {
        ...
        const host = root.querySelector('[data-region="practice-table"]');
        if (!host) {
            return;
        }
        ...
        const head = '<tr>' +
            ...
            '<th class="nxr-pr-num">' + esc(label(root, 'xp', 'Practice XP')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'totalxp', 'Total XP')) + '</th>' +
            '<th class="nxr-pr-num">' + esc(label(root, 'bonusxp', 'Bonus XP')) + '</th>' +
        ...
        const body = rows.map(function(r) {
            return '<tr>' +
                ...
                '<td class="nxr-pr-num">' + esc(r.practicexp != null ? r.practicexp : 0) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.xp) + '</td>' +
                '<td class="nxr-pr-num">' + esc(r.bonusxp != null ? r.bonusxp : 0) + '</td>' +
    ...
    const init = function() {
        const root = document.querySelector('[data-region="nxr-practice"]');
        if (!root) {
            return;
        }
        ...
        const load = function() {
            ...
            Ajax.call([{
                methodname: 'local_nexreports_get_practice_leaderboard',
                args: {
