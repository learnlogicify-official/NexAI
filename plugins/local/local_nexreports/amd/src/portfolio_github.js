/**
 * NexReports Portfolio — GitHub contribution / repo leaderboard.
 *
 * @module     local_nexreports/portfolio_github
...
define(['core/ajax', 'local_nexreports/table_export'], function(Ajax, TableExport) {
    ...
    const num = function(v) {
        return '<td class="nxr-pgh-num">' + esc(v != null ? v : 0) + '</td>';
    };
    ...
    const render = function(root, data) {
        ...
        const host = root.querySelector('[data-region="portfolio-github-table"]');
        if (!host) {
            return;
        }
        ...
        const head = '<tr>' +
            ...
            '<th>' + esc(label(root, 'login', 'GitHub')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'contributions', 'Contributions')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'commits', 'Commits')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'prs', 'PRs')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'issues', 'Issues')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'reviews', 'Reviews')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'repos', 'Repos')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'followers', 'Followers')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'following', 'Following')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'stars', 'Stars')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'forks', 'Forks')) + '</th>' +
            '<th class="nxr-pgh-num">' + esc(label(root, 'projects', 'Projects')) + '</th>' +
            '<th>' + esc(label(root, 'lastfetch', 'Last updated')) + '</th>' +
            '</tr>';

        const body = rows.map(function(r) {
            const profile = r.profileurl || ('https://github.com/' + (r.login || ''));
            return '<tr>' +
                '<td>' + esc(r.rank) + '</td>' +
                '<td class="nxr-pgh-learner"><a href="' + esc(r.url) + '">' + esc(r.firstname || '') + '</a></td>' +
                '<td>' + esc(r.lastname || '') + '</td>' +
                '<td>' + esc(r.username || '') + '</td>' +
                '<td>' + esc(r.institution || dash) + '</td>' +
                '<td>' + esc(r.yearofpassing || dash) + '</td>' +
                '<td>' + esc(r.department || dash) + '</td>' +
                '<td class="nxr-pgh-login"><a href="' + esc(profile) + '" target="_blank" rel="noopener">@' +
                esc(r.login || '') + '</a></td>' +
                num(r.contributionsyear) +
                num(r.commitsyear) +
                num(r.prsyear) +
                num(r.issuesyear) +
                num(r.reviewsyear) +
                ...
                num(r.forksreceived) +
                num(r.projectcount) +
                '<td>' + esc(r.lastfetch || dash) + '</td>' +
                '</tr>';
        }).join('');

        host.innerHTML =
            '<div class="nxr-table-wrap nxr-table-wrap--portfolio-github">' +
            '<table class="nxr-table nxr-table--portfolio-github">' +
            '<thead>' + head + '</thead><tbody>' + body + '</tbody></table></div>';
    };

    const init = function() {
        const root = document.querySelector('[data-region="nxr-portfolio-github"]');
        if (!root) {
            return;
        }
        ...
        const load = function() {
            ...
            Ajax.call([{
                methodname: 'local_nexreports_get_portfolio_github',
                args: {
