...
$PAGE->set_url(new moodle_url('/local/nexreports/course_activities.php', [
...
$PAGE->set_title(get_string('courseactivitiessummary', 'local_nexreports'));
...
$PAGE->requires->js_call_amd('local_nexreports/course_activities', 'init', [
    ['courseid' => $courseid, 'year' => $year, 'department' => $department, 'institution' => $institution],
]);

$shell = local_nexreports_shell_context('courseactivities', false);
$exporturl = (new moodle_url('/local/nexreports/download.php', [
    'report' => 'course_activities_summary',
    'format' => 'csv',
]))->out(false);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexreports/course_activities', array_merge($shell, [
    'exporturl' => $exporturl,
