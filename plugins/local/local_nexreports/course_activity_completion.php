...
$PAGE->set_url(new moodle_url('/local/nexreports/course_activity_completion.php', [
...
$PAGE->set_title(get_string('courseactivitycompletion', 'local_nexreports'));
...
$PAGE->requires->js_call_amd('local_nexreports/course_activity_completion', 'init', [
    ['courseid' => $courseid, 'cmid' => $cmid, 'year' => $year, 'department' => $department, 'institution' => $institution],
]);

$shell = local_nexreports_shell_context('courseactivitycompletion', false);
$exporturl = (new moodle_url('/local/nexreports/download.php', [
    'report' => 'course_activity_completion',
    'format' => 'csv',
]))->out(false);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexreports/course_activity_completion', array_merge($shell, [
    'exporturl' => $exporturl,
