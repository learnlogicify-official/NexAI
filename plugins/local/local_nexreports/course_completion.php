...
$PAGE->set_url(new moodle_url('/local/nexreports/course_completion.php', [
...
$PAGE->set_title(get_string('coursecompletion', 'local_nexreports'));
...
$PAGE->requires->js_call_amd('local_nexreports/course_completion', 'init', [
    ['courseid' => $courseid, 'year' => $year, 'department' => $department, 'institution' => $institution],
]);

$shell = local_nexreports_shell_context('coursecompletion', false);
...
echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexreports/course_completion', array_merge($shell, [
    'exporturl' => $exporturl,
