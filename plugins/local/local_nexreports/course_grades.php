...
/**
 * NexReports — Full course grades (sections × activities).
 *
...
$PAGE->set_url(new moodle_url('/local/nexreports/course_grades.php', [
...
$PAGE->set_title(get_string('coursegrades', 'local_nexreports'));
...
$PAGE->requires->js_call_amd('local_nexreports/course_grades', 'init', [
    ['courseid' => $courseid, 'year' => $year, 'department' => $department, 'institution' => $institution],
]);

$shell = local_nexreports_shell_context('coursegrades', false);
$exporturl = (new moodle_url('/local/nexreports/download.php', [
    'report' => 'course_grades',
    'format' => 'csv',
]))->out(false);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexreports/course_grades', array_merge($shell, [
    'exporturl' => $exporturl,
