...
$PAGE->set_url(new moodle_url('/local/nexreports/learner_course_progress.php', [
    'userid' => $userid ?: null,
]));
$PAGE->set_context($context);
$PAGE->set_title(get_string('learnercourseprogress', 'local_nexreports'));
...
$PAGE->requires->js_call_amd('local_nexreports/learner_course_progress', 'init', [
    ['userid' => $userid],
]);

$shell = local_nexreports_shell_context('learnercourseprogress', false);
$exporturl = (new moodle_url('/local/nexreports/download.php', [
    'report' => 'learner_course_progress',
    'format' => 'csv',
]))->out(false);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_nexreports/learner_course_progress', array_merge($shell, [
    'exporturl' => $exporturl,
